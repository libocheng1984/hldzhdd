...
function MainWindow(){
	...
	var mobileLayerVisibility = true;
	...
	this.createAllLayer = function(){
		...
		p.mapObj.addLayers([orgLayer,fireLayer,kkLayer,dzwlLayer,jwgzzLayer,hylskdLayer,nbdwLayer,wxpLayer]);
		
		p.mapObj.addLayers([cameralayer,eventLayer,m350Layer,mobileLayer]);
	...
	this.mapInit = function(){
		...
		*/
		...
		selector = new OpenLayers.Control.MySelectFeature([eventLayer,m350Layer,mobileLayer,fireLayer,wxpLayer,cameralayer,naturalwaterlayer,poollayer,pierlayer,cranelayer,hydrantlayer,orglayer,kklayer,dzwllayer,jwgzz,hylskd,nbdwLayer,jzwPointLayer,rwdLayer],{
			autoActivate:true,
			hover: true,
			highlightOnly:true
		});
	...
	* 加载组合定位数据
	...
	this.loadGroupCarData=function() {
		...
		$(Loader).bind("JSON_LOADED",function(e,backJson){	
         ...
         var stationgroup = p.stationGroupStore;
         var layer = groupLayer;
         
				var record = mapUtil.findRecord("hphm",backJson['value']['points'][i]['hphm'],stationgroup);
					if (record) {
						backJson['value']['points'][i]['marker'] = record['marker'];
						record = backJson['value']['points'][i];
						if(record['marker']&&record['marker']['geometry']){
							p.addDataMarker(record,true,layer);
						}else{
							p.addDataMarker(record,false,layer);
						}
						mapUtil.removeStore(stationgroup,backJson['value']['points'][i]);
						stationgroup.unshift(record);
					} else {
						//store中不存在，新增定位数据
						stationgroup.push(backJson['value']['points'][i]);
						p.addDataMarker(backJson['value']['points'][i],false,layer);
						
					}
                                        
			}
			removeGroupFeatrue(gids,p.stationGroupStore,groupLayer);
	...
	*/
	...
	this.addDataMarker = function(trafficInfo,mode,layer) {
		var lineArr = null;
		var point = null;
		...
		layer.addFeatures(featureMarker);
			//创建动画过程函数
	...
	*/
	this.loadGPSMobile=function(isFirst) {
		...
		$(Loader).bind("JSON_LOADED",function(e,backJson){	
			...
			mobileTimeout = setTimeout(function() {  p.loadGPSMobile(); }, 5000);
			//处理定位数据
	...
	*/
	...
	function addFeature(){
		...
		function removeGroupFeatrue(gids,groupFeautures,layer){
		var mapUtil = new MapUtil();
		for (var i= groupFeautures.length-1;i>=0;i--){
			var flag = false;
			for (var j=0;j<gids.length;j++){
				if(groupFeautures[i]['gid']==gids[j]){
					flag = true;
				}
			}
			if(!flag){
				mapUtil.removeAllFeatures(layer,groupFeautures[i]['marker']);
				clearPopupByFeature(groupFeautures[i]['marker']);
				groupFeautures.splice(i,1);
			}
		}
	}
