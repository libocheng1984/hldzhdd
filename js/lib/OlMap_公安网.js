...
function OlMap(){
	this.mapInitialize=function(mapId,center,zoom){
		if(!arguments[1]){
	 	  	center = new OpenLayers.LonLat(121.61761, 38.913387);
	 	}
  		if(!arguments[2]) {
  			zoom = 2;
  		}
  		var options = { 
			maxExtent: new OpenLayers.Bounds(120.7,38.5,123.7,40.4), /*设置地图范围*/
			//maxResolution: 0.0027465763646684995,
			maxResolution: 0.002746576538108565,
			numZoomLevels: 11, /*设置图层名称*/
		...
		var wmts = new OpenLayers.Layer.WMTS({
			...
			url: "http://10.80.8.234:6163/igs/rest/ogc/WMTSServer?",
			layer: "dalian:dalian",
			matrixSet: "EPSG:4490",
			matrixIds: matrixIds,
			format: "image/png",
			style: "default",
			tileOrigin: new OpenLayers.LonLat(-180,90), /*设置瓦片原点(左上角)*/
			opacity: 1,
			isBaseLayer: true
		});
		...
		mapObj.addLayers([wmts,layer,layer1/*,layer2,layer3,layer4,layer5,layer6*/,layer7]);
