...
if ($event == "newgroup") {
	...
	$count = 0;
    while(1){
      $locked = flock($fp, LOCK_NB | LOCK_EX);
      if($locked) {
      //if(true){
		...
		if($sessionId=="1"){
			...
			if ($lock_v) {
				...
				//clearstatcache(); // s清除文件状态缓存
				$currentmodif_new = date("Y-m-d H:i:s", filemtime($filename));
				clearstatcache(); // s清除文件状态缓存
				session_start();
				$_SESSION["lasttime_msg"] = $currentmodif_new;
				session_commit();
				$res = array (
					'head' => array (
						'code' => 1,
...
//	session_commit();
	...
	if ($type == "2") {
		$communication = new Communication(); //创建tpms数据库实例getImageInfo($img)
		$res = $communication->sendPicture($groupid, $_SESSION["userId"], $_SESSION["userName"], $size, $filename, $filename2,$sendName);
	} else if($type == "3"||$type == "4"||$type == "5"){
		$communication = new Communication(); //创建tpms数据库实例getImageInfo($img)
		$res = $communication->sendOthers($groupid, $_SESSION["userId"], $_SESSION["userName"], $size, $filename, $filename2,$type,$sendName);
	}else {
		$communication = new Communication(); //创建tpms数据库实例getImageInfo($img)
		$res = $communication->sendMemberGroups($groupid, $msg, $_SESSION["userId"], $_SESSION["userName"]);
	}
